...
class PluginManager:
    ...
    def fire_event(self, event_name, wait=False, *args, **kwargs):
        ...
        self.logger.info("Event %s receieved and map to method %s" % (event_name, event_method_name))
        for plugin in self._plugins:        
            event_method = getattr(plugin.object, event_method_name, None)
            if event_method:
                print ("handled by %s" % (plugin.name))
                try:
                    ...
                    print ("Method '%s' on plugin '%s' is not a coroutine" %
                                      (event_method_name, plugin.name))
            else:
                print ("No method %s mapped for event %s in plugger %s" % (event_method_name, event_name, plugin.name))
